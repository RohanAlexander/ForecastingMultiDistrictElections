...
Australia's election system follows a preferenctial voting system, in which an instant runoff election is held for each election. Estimating the distribution of preference flows from eliminated candidates is crucial to predicting the results Australian elections. It is easy to do this in a deterministic way, for instance, by assuming that what happened last election happens again, or to assume some other reasonable sounding but similar approach. We argue that a useful forecasting model would account for preferences in a probabilistic way.
...
While the tendancy to downplay uncertainty is understandable given the constraints faced by journalists [@ricketson2019like] and the difficulty that even experts have appreciating the nuances of statistical concepts [@hoekstra2018improving; @greenland2016statistical], the results of a useful forecasting model should not require statistical fluency to understand. Misunderstandings on the part of consumers of estimates are, to a large extent, the responsibility of those who produce estimates. To make the impact of uncertainty clear, communication of a model should explicitly account for areas of uncertainty in layman's terms by, for example, reporting districts that are too close to call. 
